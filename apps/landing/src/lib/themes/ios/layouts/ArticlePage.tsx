...
import { themes } from '@/lib/themes'; // Assuming themes are imported from here
...
export function ArticlePage({ article, relatedArticles, prevArticle, nextArticle, categories, basePath = '/insights' }: BaseLayoutProps) {
             ...
             <article className={cn(
                ...
                <ReactMarkdown 
                    ...
                    components={{
                        ...
                        thead: ({node, ...props}) => (
                            <thead {...props} className="bg-gray-50 dark:bg-zinc-900/50" />
                        ),
                        th: ({node, ...props}) => (
                            <th {...props} className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider" />
                        ),
                        td: ({node, ...props}) => (
                            <td {...props} className="px-6 py-4 whitespace-normal text-sm" />
                        ),
                        tr: ({node, ...props}) => (
                            <tr {...props} className="even:bg-gray-50/50 dark:even:bg-zinc-900/30" />
                        )
                    }}
                >
                    {article.content}
