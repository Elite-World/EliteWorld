...
import { Article, Category } from '@/lib/types/content'; // Correct import path
import fs from 'fs';
import path from 'path';
import matter from 'gray-matter';

// Fixed path for Markdown content
// Note: We might want to make this configurable per instance if we had multiple MD folders
const DEFAULT_DIR = path.join(process.cwd(), 'src/content/insights');
...
export class MarkdownProvider implements ContentProvider {
  ...
  constructor(options?: MarkdownProviderOptions) {
    this.folderPath = options?.folderPath 
      ? (path.isAbsolute(options.folderPath) ? options.folderPath : path.join(process.cwd(), options.folderPath))
      : DEFAULT_DIR;
  }
