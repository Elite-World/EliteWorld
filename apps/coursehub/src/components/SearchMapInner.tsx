...
function MapBoundsListener({ onBoundsChange }: { onBoundsChange?: (bounds: MapBounds) => void }) {
...
function CustomMapControls({ isFullscreen, toggleFullscreen }: { isFullscreen: boolean, toggleFullscreen: () => void }) {
  const map = useMap();
...
export default function SearchMapInner({ courses, center, onBoundsChange, isMobileMode }: SearchMapInnerProps) {
    ...
    <>
      ...
      <div className={containerClasses}>
        ...
        >
          ...
          <CustomMapControls isFullscreen={isFullscreen} toggleFullscreen={() => setIsFullscreen(!isFullscreen)} />
          
          {courses.map((course) => {
            // If course has no real coordinates, skip it (e.g., Online)
            if (course.coordinates.lat === 0 && course.coordinates.lng === 0) return null;
            
            return (
              <Marker key={course.id} position={[course.coordinates.lat, course.coordinates.lng]}>
                <Popup>
                  <div className="font-sans">
                    <div className="font-bold text-gray-900 truncate max-w-[200px]">{course.title}</div>
                    <div className="text-sm text-gray-500">${course.price} {course.priceUnit}</div>
                  </div>
                </Popup>
              </Marker>
            );
