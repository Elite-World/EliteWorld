...
export function JurisdictionTemplate({
  ...
  relatedNews = [],
  locale,
}: JurisdictionTemplateProps) {
  const dbLocale = locale === 'zh' ? 'cn' : 'en';
  // @ts-ignore
  const countryName =
    country.translations?.[dbLocale]?.name ||
    country.name?.[dbLocale] ||
    country.name?.en ||
    'Unknown';

  const bgImage = optimizeCloudinaryUrl(country.image, 2400);
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-[#0a0a0a] transition-colors duration-300">
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="max-w-4xl">
            ...
            <p className="text-xl md:text-2xl text-gray-300 font-medium max-w-2xl">
              {locale === 'zh'
                ? `在 ${countryName} 建立业务的战略情报与流动途径。`
                : `Strategic intelligence and mobility pathways for establishing presence in ${countryName}.`}
            </p>
          </div>
        </div>
      </section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-white dark:bg-[#111] border border-gray-200 dark:border-white/10 rounded-3xl p-8 shadow-xl">
                ...
                <div className="mb-8">
                  ...
                  <p className="text-sm font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mt-1">
                    {locale === 'zh' ? '免签目的地' : 'Visa-Free Destinations'}
                  </p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  {[
                    {
                      label: locale === 'zh' ? '申根区' : 'Schengen Area',
                      access: profile.passport_power.access_to_schengen,
                    },
                    {
                      label: locale === 'zh' ? '美国' : 'United States',
                      access: profile.passport_power.access_to_us,
                    },
                    {
                      label: locale === 'zh' ? '英国' : 'United Kingdom',
                      access: profile.passport_power.access_to_uk,
                    },
                    {
                      label: locale === 'zh' ? '中国' : 'China',
                      access: profile.passport_power.access_to_china,
                    },
                  ].map((region, idx) => (
                ...
                <div className="grid grid-cols-2 gap-y-8 gap-x-4">
                  <div>
                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">
                      {locale === 'zh' ? '企业税' : 'Corporate Tax'}
                    </p>
                    <p className="text-2xl font-black text-gray-900 dark:text-white">
                      {profile.tax_profile.corporate_tax}
                    </p>
                  </div>
                  <div>
                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">
                      {locale === 'zh' ? '个人所得税' : 'Personal Income'}
                    </p>
                    <p className="text-2xl font-black text-gray-900 dark:text-white">
                      {profile.tax_profile.personal_tax}
                    </p>
                  </div>
                  <div>
                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">
                      {locale === 'zh' ? '资本利得税' : 'Capital Gains'}
                    </p>
                    <p className="text-xl font-bold text-gray-700 dark:text-gray-300">
                      {profile.tax_profile.capital_gains}
                    </p>
                  </div>
                  {profile.tax_profile.crypto_tax && (
                    <div>
                      <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">
                        {locale === 'zh' ? '加密货币税' : 'Crypto Tax'}
                      </p>
                      <p className="text-xl font-bold text-gray-700 dark:text-gray-300">
                        {profile.tax_profile.crypto_tax}
                      </p>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-12">
            <h2 className="text-3xl font-black text-gray-900 dark:text-white uppercase tracking-tighter">
              {locale === 'zh'
                ? `${countryName} 的移民方案`
                : `Mobility Solutions in ${countryName}`}
            </h2>
          ...
          {solutions.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {solutions.map((sol) => {
                // @ts-ignore
                const solName =
                  sol.translations?.[dbLocale]?.name ||
                  sol.name?.[dbLocale] ||
                  sol.name?.en ||
                  'Unknown';
                // @ts-ignore
                const solDesc =
                  sol.translations?.[dbLocale]?.description ||
                  sol.description ||
                  '';
                // @ts-ignore
                const solReq =
                  sol.translations?.[dbLocale]?.requirements ||
                  sol.requirements ||
                  {};
                ...
                return (
                  ...
                  >
                    ...
                    <div className="space-y-4 pt-6 border-t border-gray-100 dark:border-white/10">
                      <div className="flex justify-between items-center">
                        <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">
                          {locale === 'zh' ? '投资额' : 'Investment'}
                        </span>
                        <span className="text-sm font-bold text-gray-900 dark:text-white">
                          {solReq.investment_amount ||
                            (locale === 'zh' ? '视情况而定' : 'Varies')}
                        </span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">
                    ...
                    <div className="mt-8 pt-4">
                      ...
                      <Link
                        href={`/programs/${(sol.name?.en || solName)
                          .toLowerCase()
                          .replace(/[^a-z0-9]+/g, '-')
                          .replace(/(^-|-$)/g, '')}-${sol._id}`}
                        className="w-full py-4 rounded-xl bg-gray-50 dark:bg-white/5 text-gray-900 dark:text-white text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 group-hover:bg-blue-600 group-hover:text-white transition-colors"
                      >
            ...
            <div className="p-12 border border-dashed border-gray-300 dark:border-white/20 rounded-3xl text-center">
              <p className="text-gray-500 dark:text-gray-400 font-medium">
                {locale === 'zh'
                  ? '该目的地目前没有发布的移民方案。'
                  : 'No mobility solutions are currently published for this jurisdiction.'}
              </p>
            </div>
          )}
        </div>
      ...
      <section className="py-24 bg-white dark:bg-[#111] border-t border-gray-100 dark:border-white/5">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-12">
            <h2 className="text-3xl font-black text-gray-900 dark:text-white uppercase tracking-tighter">
              {locale === 'zh' ? '目的地' : 'Jurisdiction'}{' '}
              <span className="text-transparent bg-clip-text bg-linear-to-r from-blue-600 to-purple-600">
                {locale === 'zh' ? '新闻与动态' : 'News & Updates'}
              </span>
            </h2>
            <div className="w-16 h-1 bg-linear-to-r from-blue-600 to-purple-600 mt-4" />
          </div>
