...
export default async function TaxHeatmapPage() {
  const data = await getTaxHeatmapData();

  return (
    <div className="min-h-screen bg-gray-50 dark:bg-[#0a0a0a]">
      <HeroSection 
        title="Global Tax Heatmap"
        subtitle="Visualize and compare the tax burdens of elite jurisdictions. Find the optimal strategic environment for your wealth structuring."
          ...
          <div className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              <h2 className="text-3xl font-black text-gray-900 dark:text-white uppercase tracking-tighter">
                Tax <span className="text-transparent bg-clip-text bg-linear-to-r from-blue-600 to-purple-600">Landscape</span>
              </h2>
            ...
            <div className="flex flex-wrap items-center gap-4 text-xs font-bold uppercase tracking-widest text-gray-500">
              <span className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-emerald-500"></div> Favorable (0-10%)</span>
              <span className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-yellow-500"></div> Moderate (11-20%)</span>
              <span className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-orange-500"></div> High (21-30%)</span>
              <span className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-red-500"></div> Restrictive (31%+)</span>
            </div>
          </div>

          <TaxHeatmapTable data={data} />
