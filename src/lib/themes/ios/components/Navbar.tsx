...
const subdomains = {
  immigration: { name: 'Elite Immi', href: 'https://immi.eliteworld.top' },
  education: { name: 'Elite Edu', href: 'https://edu.eliteworld.top' }
} as const;
    ...
    <div className="relative group">
      ...
      >
        {navigationIcons[item.id] && (
          <span>{navigationIcons[item.id]}</span>
        )}
          ...
          {item.children?.map(child => (
            ...
            >
              {navigationIcons[child.id] && (
                <span className="mr-2">{navigationIcons[child.id]}</span>
              )}
              {child.label}
            </Link>
          ))}
...
function MenuLink({ item, mobile = false }: { item: NavigationItem; mobile?: boolean }) {
  ...
  if (!mobile) {
    return (
      ...
      >
        {navigationIcons[item.id] && (
          <span className="mr-2">{navigationIcons[item.id]}</span>
        )}
        {item.label}
      </Link>
    );
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="relative group">
            ...
            <div className={cn(
              ...
              {Object.values(subdomains).map((site) => (
                <a
                  key={site.href}
                  href={site.href}
                  target="_blank"
