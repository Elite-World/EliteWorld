...
export function Navbar({
  ...
  const open = (view: string, data?: any) => {
    console.log('Open modal', view, data);
  };
        ...
        <div className="flex items-center justify-between h-16">
          <div className="relative group">
            <div
              ...
              className={cn(
                'cursor-pointer',
                'text-lg font-semibold relative transition-colors',
            ...
            {navGateway && Object.keys(navGateway).length > 1 && (
              ...
              >
                {Object.entries(navGateway)
                  .filter(([_, item]) => item.name !== siteConfig.name)
                  .map(([key, item]) => (
                    <a
                      key={key}
                      href={item.href}
